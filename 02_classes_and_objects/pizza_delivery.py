class PizzaDelivery:
	...
	def add_extra(self, ingredient: str, quantity: int,
			price_per_quantity: float) -> str | None:
		if self.ordered:
			return (
				f"Pizza {self.name} already prepared, and we can't make any changes!")
		
		self.ingredients[ingredient] = self.ingredients.get(ingredient,
			0) + quantity
