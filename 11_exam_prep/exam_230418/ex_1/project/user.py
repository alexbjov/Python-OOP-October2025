class User:
    RATING_INCREMENT: float = 0.5
    RATING_DECREMENT: float = 2.0
    ...
    def increase_rating(self) -> None:
        self.rating += self.RATING_INCREMENT
        if self.rating > 10:
            self.rating = 10
    
    def decrease_rating(self) -> None:
        if self.rating < self.RATING_DECREMENT:
            self.rating = 0
            self.is_blocked = True
        else:
            self.rating -= self.RATING_DECREMENT
    
    def __str__(self):
        return (f"{self.first_name} {self.last_name} Driving license: {self.driving_license_number} Rating: "
                f"{self.rating}")
